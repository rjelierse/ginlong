...
const (
	...
	upstreamAddress = "47.88.8.200:10000"
)
...
func main() {
	var cfg config
	flag.StringVar(&cfg.ListenAddress, "listen", listenAddress, "The address to listen on for incoming data")
	flag.StringVar(&cfg.UpstreamAddress, "upstream", upstreamAddress, "The address to relay messages to")
	flag.Parse()
	...
	// Run proxy server.
	server := proxy.New(cfg.ListenAddress, cfg.UpstreamAddress)
	go func() {
